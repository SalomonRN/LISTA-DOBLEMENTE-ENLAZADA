 ...
 */
void createOne(Node *&head, int value, Node *&last)
{
    ...
    newNode->previous = nullptr; // Se actualiza el valor de 'newNode.next' a 'nullptr'
 ...
 * @param newNode Variable puntero que hace referencia al objeto, y que guarda la posicion de memoria.
 */
void createInPosition(int value, Node *&head, int position, Node *&last)
{
    Node *newNode = new Node; // Se crea un nuevo Nodo, y '*newNode' guarda la posicion en memoria del objeto. (Puntero)
    newNode->data = value;    // Se actualiza el valor de 'newNode.data' segun 'value'
    newNode->next = nullptr;  // Se actualiza el valor de 'newNode.next' a 'nullptr'
...
void deleteOne(Node *&head, int filter, int position)
{
    ...
    Node *aux = NULL;     // Se crea un puntero para de tipo Nodo iniciado en null.
    int i = 0;            // Variable para llevar el indice actual.
...
void deleteFirts(Node *&head, int filter)
{
    ...
    Node *aux = NULL;     // // Se crea un nuevo Node 'past' para guardar el Nodo anterior.
 ...
 * @param past  Variable node para guardar el nodo anterior para despues actualizar su apuntador
 */
void deleteAll(Node *&head, int filter, Node *&last)
{
    Node *current = head; // Se crea un nuevo Node 'current' para moverse entre posiciones.
    Node *past = NULL;    // // Se crea un nuevo Node 'past' para guardar el Nodo anterior.
    Node *aux = NULL;     // // Se crea un nuevo Node 'past' para guardar el Nodo anterior.
    ...
    if (head != NULL)
    {
        ...
        while (current != nullptr)
        {
            ...
            if (current->data == filter && current == head)
            {
                ...
                head->previous = nullptr;
                delete current;
                cout << "Se eliminó un dato de la cabeza" << endl;
                current = head;
            }
            // Para cuando la cabeaza no tiene el valor que sea desea borrar
            else
            {
                if (current->data == filter && current == last)
                {
                    last = past;
                    last->next = nullptr;
                    aux = current;
                    delete aux;
                    cout << "Se eliminó dato de la cola" << endl;
                    break;
                }
...
int main()
{
    ...
    Node *last = nullptr; // Se crea un puntero de tipo Nodo vacio.
    ...
    do
    {
        ...
        cout << "10. ReadLast: Para leer la lista desde el final" << endl;
        cin >> choise;
        ...
        case 1:
            ...
            cout << "Posicion: ";
            cin >> position;
            createInPosition(data, head, position, last);
            break;
        case 2:
            cout << "Dato a buscar: ";
            cin >> data;
